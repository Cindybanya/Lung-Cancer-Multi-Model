...
def extract_features_and_labels(ct_folder, mask_folder, merged_df, device):
    features = []
    labels = []
    clinical_data = [] 
    ...
    for patient_folder in os.listdir(ct_folder):
        ...
        mask_resized = Image.fromarray(mask_slice.astype(np.float32)).resize((1024, 1024), Image.BILINEAR)
        mask_tensor = torch.as_tensor(np.array(mask_resized).astype('float')).unsqueeze(0).unsqueeze(0).to(device) # [1, 1, 64, 64]
        # expend mask
        mask_expanded = mask_tensor.expand(-1, 3, -1, -1)  # 扩展 mask 到 [batch_size, 256, 1024, 1024]

        # read file
        dicom_data = pydicom.dcmread(dicom_file_path)
            
        image_array = dicom_data.pixel_array.astype(np.uint8)  # 确保数据为 uint8 类型
        image = Image.fromarray(image_array).convert("L")  # 转换为灰度图像
        image_rgb = Image.merge("RGB", (image, image, image))  # 转换为 RGB 三通道
        ...
        #  mask x vision_features 
        combined_features = img_tensor * mask_expanded

        combined_features = combined_features.to(torch.float32)

        conv_layer = nn.Conv2d(in_channels=3, out_channels=256, kernel_size=3, stride=1, padding=1)

        # Apply convolution
        conv_output = conv_layer(combined_features)

        # Step 2: Apply Average Pooling to reduce spatial dimensions from 1024x1024 to 64x64
        avg_pool_layer = nn.AvgPool2d(kernel_size=16, stride=16)

        # Apply average pooling
        combined_features = avg_pool_layer(conv_output)
...
features, labels,clinic = extract_features_and_labels(
    source_ct_folder, source_mask_folder, merged_df_all, device
)

save_path = "/nosam_processed_241_data.pt"
torch.save({
    "features": features,
    "labels": labels,
    "clinic": clinic
...
data_path = "/nosam_processed_241_data.pt"
